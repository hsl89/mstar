...
def relative_position_bucket(relative_position,
    ...
    val_if_large = max_exact + (
            th.log(relative_position.type(th.float32) / max_exact)
            / math.log(max_distance / max_exact) * (num_buckets - max_exact)).astype(th.int32)
...
def get_positional_embedding(units, max_length=None,
...
class SinusoidalPositionalEmbedding(nn.Module):
    def __init__(self, units: int, learnable=False):
        ...
        self.freq = nn.Parameter(data=th.tensor(default_sinusoidal_base),
