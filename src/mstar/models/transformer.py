...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.attn_qkv = nn.Linear(out_features=3 * units,
                                  in_features=units,
                                  bias=use_qkv_bias)
        self.attention_proj = nn.Linear(out_features=units,
                                        in_features=units,
                                        bias=True)
...
class TransformerDecoderLayer(nn.Module):
    def __init__(self, units: int = 512,
        ...
        self.attn_in_qkv = nn.Linear(out_features=3 * units,
                                     in_features=units,
                                     bias=use_qkv_bias)
        ...
        self.attn_inter_q = nn.Linear(out_features=units,
                                      in_features=units,
                                      bias=use_qkv_bias)
        self.attn_inter_k = nn.Linear(out_features=units,
                                      in_features=mem_units,
                                      bias=use_qkv_bias)
        self.attn_inter_v = nn.Linear(out_features=units,
                                      in_features=mem_units,
                                      bias=use_qkv_bias)
        self.inter_attention = MultiHeadAttentionCell(query_units=units,
                                                      ...
                                                      attention_dropout=self._attention_dropout,
                                                      layout=attention_layout)
        self.proj_inter = nn.Linear(in_features=units,
                                    out_features=units,
                                    bias=True)
        self.ln_in = nn.LayerNorm(eps=layer_norm_eps,
                                  normalized_shape=units)
        self.ln_inter = nn.LayerNorm(eps=layer_norm_eps,
                                     normalized_shape=units)
        self.ffn = PositionwiseFFN(units=units,
                                   hidden_size=hidden_size,
                                   dropout=dropout,
    ...
    def init_states(self, batch_size, device=None, dtype='float32'):
        ...
        if self.layout == 'NT':
            init_key = th.zeros(size=(batch_size, 0, self._num_heads,
            ...
            init_key = th.zeros(size=(0, batch_size, self._num_heads,
                                      self._units // self._num_heads),
                                device=device, dtype=dtype)
...
class TransformerDecoder(nn.Module):
    def __init__(self, num_layers=6,
                 ...
                 hidden_size=2048, use_qkv_bias=True,
                 num_heads=8, max_shift=None, activation_dropout=0.0,
                 dropout=0.1, attention_dropout=0.1,
                 gated_proj: bool = False,
                 layer_norm_eps=1E-5, data_norm=False,
                 pre_norm=False, activation='relu', layout='NT'):
        super().__init__()
        self._units = units
        self._mem_units = mem_units
