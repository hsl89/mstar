    ...
    def forward(
        ...
        for unique_task_id in unique_task_ids_list:
            ...
            logits, loss = self.heads[decoder_id](
                    ...
                    description_output=description_output[current_task_filter],
...
class TokenClassificationDecoder(nn.Module):
    ...
    def forward(self, sequence_output, attention_mask, description_output=None, labels=None, **kwargs):
        ...
        # Unsqueeze description_output to (batch_size) x (hidden size) x 1, repeat it to (batch_size) x (hidden size) x (sequence length)
        # Transpose this output to (batch size) x (sequence length) x (hidden size)
        # Concatenate with sequence output on dimension 2 to give (batch size) x (sequence length) x (hidden size * 2)
        # pdb.set_trace()
        description_output = description_output.unsqueeze(2)
        description_output = description_output.repeat(1, 1, sequence_output.shape[1])
        description_output = description_output.transpose(1, 2)
        concat_output = torch.cat((description_output, sequence_output), 2)
        # pdb.set_trace()
        logits = self.model(concat_output)
        # pdb.set_trace()
