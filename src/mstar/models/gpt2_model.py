...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False):
        ...
        # addition for Alibi - attention with linear biases (https://arxiv.org/pdf/2108.12409.pdf)
        if self.positional_embedding == 'alibi':
            self.alibi = compute_alibi_bias(max_positions, self.num_heads)
...
class GPT2Config(PretrainedConfig):
    ...
    Args:
        ...
        positional_embedding (:obj:`str`, `optional`, defaults to :obj:`absolute`):
            Which type of positional embedding to use, currently supported `absolute` or `alibi`
    ...
    def __init__(
        ...
        **kwargs
        ...
        assert positional_embedding in [
            "absolute",
            "alibi",
        ], f"Positional embedding `{positional_embedding}` is not supported"
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        if self.positional_embedding == 'absolute':
            ...
            raise NotImplementedError(f"Positional embedding `{self.positional_embedding}` is \
            not supported. Use one of `absolute`, `alibi`")
...
class GeLUFunction(torch.autograd.Function):
    ...
    def forward(ctx, input_val, bias):
        ctx.save_for_backward(input_val, bias)
        return bias_gelu(bias, input_val)

    @staticmethod
    def backward(ctx, grad_output):
        input_val, bias = ctx.saved_tensors
        tmp = bias_gelu_back(grad_output, bias, input_val)
        return tmp, tmp
