"""Code to preprocess data for fine tuning a simple model"""
import math
import tqdm
import argparse
import os
...
def process_file(path_pair):
    ...
    for document_data in document_segmented_data:
        ...
        for i in range(math.ceil(len(all_toks) / target_seq_length)):
            ...
            # Arrays in same order as schema
            buffers[0].append(final_toks)
            buffers[1].append(len(final_toks))
            # buffers[2].append([])#mlmpositions)
            # buffers[3].append([])#mlmlabels)
            # buffers[0] = final_toks
            buffers[2].append(final_seqtagging_labels)
