...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False):
        ...
        if masked_softmax_fusion:
            _load_megatron_kernels()
            from megatron.model.fused_softmax import FusedScaleMaskSoftmax
            from megatron.model.enums import AttnMaskType

            if self.scale_attn_weights:
                self.scale_attn_factor = 1.0 / (float(self.head_dim) ** 0.5)
            else:
            ...
            attention_softmax_in_fp32 = (
                True  # Only takes effect for the unfused fallback kernel
            )
            self.scale_mask_softmax = FusedScaleMaskSoftmax(
                fp16,
                bf16,  # Only takes effect for the unfused fallback kernel
                attn_mask_type,
    ...
    def _attn_fused(self, query, key, value, attention_mask=None, head_mask=None):
        ...
        attn_weights = (
            ...
            .baddbmm_(query, key, beta=0.0, alpha=self.scale_attn_factor)
            .view(*output_size)
        )

        if (
            not self.is_cross_attention
            and not self.scale_mask_softmax.is_kernel_available(
                attention_mask, *output_size
            )
        ):
            # if only "normal" attention layer implements causal mask
            causal_mask = self.bias[
            ...
            attention_mask_func(attn_weights, causal_mask)
    ...
    def forward(
        ...
        if self.masked_softmax_fusion:
            attn_output, attn_weights = self._attn_fused(
                query, key, value, attention_mask, head_mask
            )
        else:
            attn_output, attn_weights = self._attn_torch(
                query, key, value, attention_mask, head_mask
            )
...
def attention_mask_func(attention_scores, attention_mask):
    attention_mask = attention_mask.bool().logical_not_()
    attention_scores.masked_fill_(attention_mask, -10000.0)
    return attention_scores


def _load_megatron_kernels():
    from collections import namedtuple
    from megatron import fused_kernels

    args = namedtuple("Arguments", ["rank", "masked_softmax_fusion"])(0, True)
